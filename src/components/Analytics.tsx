      ...
      <main className="max-w-6xl mx-auto px-3 sm:px-4 py-4 sm:py-8">
        {sales.length === 0 ? (
          <div className="bg-white rounded-lg shadow-md p-8 sm:p-12 text-center">
            <p className="text-sm sm:text-lg text-gray-500">No data available for analytics</p>
          </div>
        ) : (
          <>
            {/* Key Metrics Grid */}
            <div className="grid grid-cols-2 lg:grid-cols-4 gap-2 sm:gap-4 mb-6">
              ...
              <div className="bg-white rounded-xl p-3 sm:p-4 border border-gray-100 shadow-sm">
                <div className="flex items-center gap-1.5 sm:gap-2 mb-1.5 sm:mb-2">
                  ...
                  <span className="text-[10px] sm:text-xs font-medium text-gray-500 uppercase">Total Packs</span>
                </div>
              ...
              <div className="bg-white rounded-xl p-3 sm:p-4 border border-gray-100 shadow-sm">
                <div className="flex items-center gap-1.5 sm:gap-2 mb-1.5 sm:mb-2">
                  <span className="text-sm sm:text-lg">💰</span>
                  <span className="text-[10px] sm:text-xs font-medium text-gray-500 uppercase">Revenue</span>
                </div>
              ...
              <div className="bg-white rounded-xl p-3 sm:p-4 border border-gray-100 shadow-sm">
                <div className="flex items-center gap-1.5 sm:gap-2 mb-1.5 sm:mb-2">
                  <span className="text-sm sm:text-lg">⏳</span>
                  <span className="text-[10px] sm:text-xs font-medium text-gray-500 uppercase">Pending</span>
                </div>
              ...
              <div className="bg-white rounded-xl p-3 sm:p-4 border border-gray-100 shadow-sm">
                <div className="flex items-center gap-1.5 sm:gap-2 mb-1.5 sm:mb-2">
                  <span className="text-sm sm:text-lg">📊</span>
                  <span className="text-[10px] sm:text-xs font-medium text-gray-500 uppercase">Avg Packs/Day</span>
                </div>
            ...
            {stats.bestSellingDay && (
              <div className="bg-gradient-to-r from-[#073011] to-[#2e823f] rounded-xl p-3 sm:p-6 text-white mb-6">
                <div className="flex items-center justify-between">
                  <div>
                    <p className="text-green-200 text-xs sm:text-sm mb-1">🏆 Best Selling Day</p>
                  ...
                  <div className="text-right">
                    ...
                    <p className="text-green-200 text-xs sm:text-sm">packs sold</p>
                  </div>
                </div>
              </div>
            )}
            ...
            <div className="bg-white rounded-xl p-3 sm:p-6 border border-gray-100 shadow-sm mb-6">
              <h3 className="text-xs sm:text-sm font-medium text-gray-500 uppercase tracking-wide mb-3 sm:mb-4">Payment Status</h3>
              <div className="grid grid-cols-2 gap-2 sm:gap-4">
                <div className="bg-green-50 rounded-xl p-3 sm:p-4 text-center">
                  <p className="text-green-600 text-[10px] sm:text-xs font-medium mb-1">Paid</p>
                  <p className="text-base sm:text-2xl font-bold text-green-700">₹{stats.paidAmount.toLocaleString()}</p>
                </div>
                <div className="bg-orange-50 rounded-xl p-3 sm:p-4 text-center">
                  <p className="text-orange-600 text-[10px] sm:text-xs font-medium mb-1">Pending</p>
                  <p className="text-base sm:text-2xl font-bold text-orange-700">₹{stats.pendingAmount.toLocaleString()}</p>
                </div>
              </div>
            ...
            {stats.dailyData.length > 0 && (
              <div className="bg-white rounded-xl p-3 sm:p-6 border border-gray-100 shadow-sm">
                <h3 className="text-xs sm:text-sm font-medium text-gray-500 uppercase tracking-wide mb-3 sm:mb-4">Last 7 Days Performance</h3>
                <div className="space-y-2 sm:space-y-3">
                  {stats.dailyData.map((day) => (
                    <div key={day.date} className="flex items-center gap-2 sm:gap-3">
                      <span className="text-[10px] sm:text-xs text-gray-500 w-12 sm:w-16 flex-shrink-0">{formatDate(day.date)}</span>
                      <div className="flex-1 h-6 sm:h-8 bg-gray-50 rounded-lg overflow-hidden">
                        <div 
                        ...
                        >
                          <span className="text-white text-[10px] sm:text-xs font-medium">{day.packs}</span>
                        </div>
                      </div>
                      <span className="text-[10px] sm:text-xs text-gray-600 w-12 sm:w-16 text-right">₹{day.revenue.toLocaleString()}</span>
                    </div>
                  ))}
                </div>
              </div>

  ...
  return (
    <div className="fixed inset-0 bg-gray-50 z-50 overflow-y-auto">
      ...
      <main className="max-w-6xl mx-auto px-3 sm:px-4 py-4 sm:py-8">
        {/* Summary Card */}
        <div className="bg-white rounded-xl p-4 sm:p-6 border border-gray-100 shadow-sm mb-4 sm:mb-6">
              ...
              >
                <div className="flex items-center justify-between mb-3">
                  <div className="flex items-center gap-3">
                    <div className="w-10 h-10 sm:w-12 sm:h-12 bg-gradient-to-br from-[#073011] to-[#2e823f] rounded-xl flex items-center justify-center text-white font-bold text-lg">
                      {customer.name.charAt(0).toUpperCase()}
                    </div>
                    <div>
                      <h3 className="font-semibold text-gray-900 text-sm sm:text-base">{customer.name}</h3>
                      <p className="text-xs text-gray-700">{customer.totalEntries} entries</p>
                    </div>
                  </div>
                ...
                <div className="flex gap-2 text-xs">
                  <div className="flex-1 bg-green-100 rounded-lg p-2 text-center">
                    <span className="text-green-800 font-medium">Paid: ₹{customer.paidAmount.toLocaleString()}</span>
                  </div>
                  <div className="flex-1 bg-red-100 rounded-lg p-2 text-center">
                    <span className="text-red-800 font-medium">Pending: ₹{customer.pendingAmount.toLocaleString()}</span>
                  </div>
                </div>
              </div>
            ))}
